...

{
	if (ipl == 0.0F) return (0xffdf);
		...
		fprintf(stderr, "tx pl code %f is invalid\n", ipl);
}
...

{
	if (ipl == 0.0F) return (0xffdf);
		...
		fprintf(stderr, "rx pl code %.3f is invalid\n", ipl);
}

int totlookup(int mins, int secs)

{
	...
	if (i > 31)
		fprintf(stderr, "tot value %d minutes %d seconds is too large\n",
			mins, secs);
	i = (~ i) & 0x1f;
...
void calc(unsigned int txfreq, unsigned int rxfreq, unsigned char *plugbuf,
	unsigned int txdpl, unsigned int txdplinv, unsigned int rxdpl,
	unsigned int rxdplinv, unsigned int txpl, unsigned int rxpl,
	unsigned int txmpl, unsigned int rxmpl, unsigned int tot, unsigned int pwr,
	unsigned int scantype, unsigned int tbscan, unsigned int p1scan,
	unsigned int p2scan, unsigned int squelchtype, unsigned int scansource,
	unsigned int scanlist)
...

{
	int rc;
	int i;
	char termbuf[MAXSTR+1];
	unsigned int txfreq, rxfreq;
	unsigned int txdpl, txdplinv;
	unsigned int rxdpl, rxdplinv;
	float ipl;
	unsigned int txpl, rxpl;
	unsigned int txmpl, rxmpl;
	unsigned int tomins, tosecs, tot;
	unsigned int pwr;
	unsigned int scantype;
	unsigned int tbscan;
	unsigned int squelchtype;
	unsigned int p1scan, p2scan;
	unsigned int scansource;
	unsigned char plugbuf[16];
	unsigned int mode;
	unsigned int scanlist;

	while (! feof(stdin))
	{
		rc = fscanf(stdin, "%s", termbuf);
		if (strcasecmp(termbuf, "MODE") != 0)
		{
			if (rc == EOF)
				break;
			fprintf(stderr, "input problem: expected MODE, got %s\n", termbuf);
			exit(1);
		}

		rc = fscanf(stdin, "%d", &txfreq);

		rc = fscanf(stdin, "%d", &rxfreq);

		rc = fscanf(stdin, "%o", &txdpl);

		rc = fscanf(stdin, "%s", termbuf);
		if (strcasecmp(termbuf, "YES") == 0) txdplinv = 1;
		else if (strcasecmp(termbuf, "NO") == 0) txdplinv = 0;
		else
			fprintf(stderr, "unrecognized tx dpl inv value %s\n", termbuf);

		rc = fscanf(stdin, "%f", &ipl);
		txpl = txplookup(ipl);

		rc = fscanf(stdin, "%s", termbuf);
		if (strcasecmp(termbuf, "YES") == 0) txmpl = 1;
		else if (strcasecmp(termbuf, "NO") == 0) txmpl = 0;
		else
			fprintf(stderr, "unrecognized tx mpl value %s\n", termbuf);

		rc = fscanf(stdin, "%o", &rxdpl);

		rc = fscanf(stdin, "%s", termbuf);
		if (strcasecmp(termbuf, "YES") == 0) rxdplinv = 1;
		else if (strcasecmp(termbuf, "NO") == 0) rxdplinv = 0;
		else
			fprintf(stderr, "unrecognized rx dpl inv value %s\n", termbuf);

		rc = fscanf(stdin, "%f", &ipl);
		rxpl = rxplookup(ipl);

		rc = fscanf(stdin, "%s", termbuf);
		if (strcasecmp(termbuf, "YES") == 0) rxmpl = 1;
		else if (strcasecmp(termbuf, "NO") == 0) rxmpl = 0;
		else
			fprintf(stderr, "unrecognized rx mpl value %s\n", termbuf);

		rc = fscanf(stdin, "%d", &tomins);
		rc = fscanf(stdin, "%d", &tosecs);
		tot = totlookup(tomins, tosecs);

		rc = fscanf(stdin, "%s", termbuf);
		if (strcasecmp(termbuf, "HIGH") == 0) pwr = 1;
		else if (strcasecmp(termbuf, "LOW") == 0) pwr = 0;
		else
			fprintf(stderr, "unrecognized tx power value %s\n", termbuf);

		rc = fscanf(stdin, "%s", termbuf);
		if (strcasecmp(termbuf, "DBLPRI") == 0) scantype = 0;
		else if (strcasecmp(termbuf, "SGLPRI") == 0) scantype = 1;
		else if (strcasecmp(termbuf, "STD") == 0) scantype = 3;
		else if (strcasecmp(termbuf, "NONE") == 0) scantype = 2;
		else
			fprintf(stderr, "unrecognized scan type value %s\n", termbuf);

		rc = fscanf(stdin, "%s", termbuf);
		if (strcasecmp(termbuf, "YES") == 0) tbscan = 1;
		else if (strcasecmp(termbuf, "NO") == 0) tbscan = 0;
		else
			fprintf(stderr, "unrecognized talkback scan value %s\n", termbuf);

		rc = fscanf(stdin, "%d", &p1scan);

		rc = fscanf(stdin, "%d", &p2scan);

		rc = fscanf(stdin, "%s", termbuf);
		if (strcasecmp(termbuf, "ANDOR") == 0) squelchtype = 0;
		else if (strcasecmp(termbuf, "ANDSTD") == 0) squelchtype = 1;
		else if (strcasecmp(termbuf, "STDSTD") == 0) squelchtype = 3;
		else
			fprintf(stderr, "unrecognized squelch type %s\n", termbuf);

		rc = fscanf(stdin, "%s", termbuf);
		if (strcasecmp(termbuf, "FIXED") == 0) scansource = 1;
		else if (strcasecmp(termbuf, "SELECTABLE") == 0) scansource = 0;
		else
			fprintf(stderr, "unrecognized scan source %s\n", termbuf);

		mode = 0;
		scanlist = 0xffffffff;
		rc = fscanf(stdin, "%s", termbuf);
		while (strcasecmp(termbuf, "END") != 0)
		{
			mode = atoi(termbuf);
			scanlist &= ~(1 << (32 - mode));
			rc = fscanf(stdin, "%s", termbuf);
		}
/*		scanlist = htonl(scanlist); */

		calc(txfreq, rxfreq, plugbuf, txdpl, txdplinv, rxdpl, rxdplinv,
			txpl, rxpl, txmpl, rxmpl, tot, pwr, scantype, tbscan, p1scan,
			p2scan, squelchtype, scansource, scanlist);

		for (i = 0; i < 16; i++)
			printf("%02X ", plugbuf[i]);
		puts("");
	}
}
